...
- 🔍 **快速文件夹搜索**：使用快捷键 `Cmd+Alt+F`（Mac）/ `Ctrl+Alt+F`（Windows/Linux）快速搜索工作区中的所有文件夹
- 📌 **固定文件夹**：将常用文件夹固定到侧边栏面板，在处理任务时快速访问
- 🔗 **符号链接支持**：跟踪符号链接并索引其内容
- 🚀 **智能缓存**：缓存结果以实现即时的后续搜索
...
**快速访问**：按 `Cmd+Alt+F`（Mac）或 `Ctrl+Alt+F`（Windows/Linux）

或者：
1. 打开命令面板（`Cmd+Shift+P` 或 `Ctrl+Shift+P`）
2. 输入 "Search Folders" 并选择 **"RAC Folder Search: Search Folders"**
3. 开始输入以过滤结果
4. 点击固定按钮（📌）将文件夹固定以便快速访问
5. 选择一个文件夹以在文件资源管理器中显示它
...
2. **查看固定的文件夹**：
   - 打开 Explorer 面板（左侧文件资源管理器）
   - 所有固定的文件夹都显示在"Pinned Folders"视图中

3. **打开固定的文件夹**：
   - 点击任何固定的文件夹以在资源管理器中打开它

4. **取消固定文件夹**：
   - 点击"Pinned Folders"视图中文件夹旁边的关闭图标
   - 或右键单击并选择"Unpin Folder"

5. **清除所有固定的文件夹**：
   - 点击"Pinned Folders"视图标题栏中的清除按钮（🗑️）
...
新增固定文件夹功能：
- **固定文件夹以便快速访问**：从搜索结果中固定常用文件夹
- **Explorer 面板集成**：固定文件夹显示在 Explorer 面板中（而不是单独的 Activity Bar 图标）
- **持久化存储**：固定的文件夹在 VS Code 会话之间保持
- **轻松管理**：取消固定单个文件夹或一次清除所有
- **视觉指示器**：符号链接文件夹显示 🔗 图标
...
- 直接从 VS Code 扩展面板安装
- 搜索 "RAC Folder Search" 进行安装
- 通过 VS Code 自动更新

### 1.0.2

重命名和增强：
- 重命名为 "RAC Folder Search"
- 添加快捷键：`Cmd+Alt+F`（Mac）/ `Ctrl+Alt+F`（Windows/Linux）
- 增强配置描述
- 完整的自动化设置
